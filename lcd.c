...
void lcd_init(void)
{
    ...
    lcd_writenibble(4 << 0b0011);  // Use lcd_writenibble to send 0b0011
    _delay_ms(5);               	 // Delay at least 4msec

    lcd_writenibble(4 << 0b0011);  // Use lcd_writenibble to send 0b0011
    _delay_us(120);                // Delay at least 100usec

    lcd_writenibble(4 << 0b0011);  // Use lcd_writenibble to send 0b0011, no delay needed

    lcd_writenibble(4 << 0b0010);  // Use lcd_writenibble to send 0b0010
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    // To make LCD read the 4 bits
    PORTB |= (1 << PB1);
	  PORTB |= (1 << PB1); // Ensures that E pulse >= 230 ns
    PORTB &= ~(1 << PB1);    
}
